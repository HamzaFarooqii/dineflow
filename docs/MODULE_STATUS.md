...
| **D. Kitchen / KDS** | 🟡 | KDS board, Fire/Mark ready/Serve ✅ | Ticket creation on every order, station grouping, item transitions ✅. A fully-served dine-in ticket now flips its table to `served` (calls `floor.ts`'s shared transition primitive directly, best-effort — see `kitchen.ts`'s `patchItem`) | `kitchen_tickets`, `kitchen_ticket_items` ✅ | orders.test.ts + kitchen-ticket-status.test.ts + floor.test.ts ✅ | Delay/SLA tracking and course-based firing not built |
