...
| **B. Front of House / Tables** | 🟡 | Floor screen, area tabs, table cards (now showing last order total), Seat/Add order/Bill/Settle/Clean ✅. Transfer/Merge 🔴 (disabled, correctly labeled) | `/floor` read + atomic status-transition PATCH ✅. `applyTableStatusTransition` now shared with the kitchen's served-ticket hook | `floor_areas`, `restaurant_tables`(+assigned_waiter_id) ✅ | `floor.test.ts` (PGlite, covers the shared transition primitive) ✅ | Reservations/waitlist not scheduled. "Last order total" is honestly a completed order, not a live running tab — no open-ticket layer exists to source one from (see floor.ts's comment) |
...
| **D. Kitchen / KDS** | 🟡 | KDS board, Fire/Mark ready/Serve ✅ | Ticket creation on every order, station grouping, item transitions ✅. A fully-served dine-in ticket now flips its table to `served` (calls `floor.ts`'s shared transition primitive directly, best-effort — see `kitchen.ts`'s `patchItem`). A manager can also trigger the same transition by hand from the Floor screen's "Mark served" button, for when the kitchen isn't the reliable source of truth — a cashier terminal cannot | `kitchen_tickets`, `kitchen_ticket_items` ✅ | orders.test.ts + kitchen-ticket-status.test.ts + floor.test.ts (covers both the automatic and manager-triggered paths, and that a terminal caller is rejected) ✅ | Delay/SLA tracking and course-based firing not built |
